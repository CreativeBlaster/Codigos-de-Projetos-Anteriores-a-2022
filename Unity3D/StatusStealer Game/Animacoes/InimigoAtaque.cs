...
public class InimigoAtaque : MonoBehaviour {
    ...
    void Start () {
        ...
        controle = 0.0F;
        movimentoMax = 0.5F;
        posInicial.Set(0.338F, -1.119F, 0F); 
        velocidade = 0.05F;
        rotina = false;
        impacto = false;
        fim = false;
	...
	void Update () {
        ...
        {
            diferenca =  posInicial.x - positionNow.x;
            caminharAtraz = Vector3.right;
            caminharFrente = Vector3.left;
        }
        ...
        if (controle >= 1.0F)//TUDO SÓ FUNCIONARA SE O BOTAO PLAY TIVER SIDO APERTADO
        {
                ...
                if (!rotina && fim) {
                    ...
                    if (transform.rotation.z >= 0.0 )//rotação 0 manda volta
                    {
                        ...
                        if (diferenca <= 0.0F)//está na posicao original, manda parar
                        {
                            ...
                            inimigo.GetComponent<RESETPosicional>().SetPlay(true);
            ...
            if (!play)// && repeticao == 1.0F)
            {
                ...
                controle = 0.0F;
                movimentoMax = 0.5F;
                //posInicial.Set(-0.3161F,-0.1213F,0F);
                velocidade = 0.05F;
                rotina = false;
                impacto = false;
                fim = false;
